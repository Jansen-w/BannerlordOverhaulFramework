...
namespace BOF.Campaign.Map.Event
{
  public class MapEventSide
  {
    ...
    internal static object AutoGeneratedGetMemberValueLeaderParty(object o) => (object) ((MapEventSide) o).LeaderParty;

    internal static object AutoGeneratedGetMemberValueMissionSide(object o) => (object) ((MapEventSide) o).MissionSide;

    internal static object AutoGeneratedGetMemberValueCasualtyStrength(object o) => (object) ((MapEventSide) o).CasualtyStrength;

    internal static object AutoGeneratedGetMemberValueStrengthRatio(object o) => (object) ((MapEventSide) o).StrengthRatio;

    internal static object AutoGeneratedGetMemberValueRenownValue(object o) => (object) ((MapEventSide) o).RenownValue;

    internal static object AutoGeneratedGetMemberValueInfluenceValue(object o) => (object) ((MapEventSide) o).InfluenceValue;

    internal static object AutoGeneratedGetMemberValueCasualties(object o) => (object) ((MapEventSide) o).Casualties;

    internal static object AutoGeneratedGetMemberValue_battleParties(object o) => (object) ((MapEventSide) o)._battleParties;

    internal static object AutoGeneratedGetMemberValue_mapEvent(object o) => (object) ((MapEventSide) o)._mapEvent;

    internal static object AutoGeneratedGetMemberValue_mapFaction(object o) => (object) ((MapEventSide) o)._mapFaction;

    internal static object AutoGeneratedGetMemberValue_selectedSimulationTroopIndex(object o) => (object) ((MapEventSide) o)._selectedSimulationTroopIndex;

    internal static object AutoGeneratedGetMemberValue_selectedSimulationTroopDescriptor(object o) => (object) ((MapEventSide) o)._selectedSimulationTroopDescriptor;

    internal static object AutoGeneratedGetMemberValue_selectedSimulationTroop(object o) => (object) ((MapEventSide) o)._selectedSimulationTroop;

    internal static object AutoGeneratedGetMemberValueIsSurrendered(object o) => (object) ((MapEventSide) o).IsSurrendered;

    internal static object AutoGeneratedGetMemberValue_nearbyPartiesAddedToPlayerMapEvent(object o) => (object) ((MapEventSide) o)._nearbyPartiesAddedToPlayerMapEvent;
    ...
    internal void DistributeLootAmongWinners(LootCollector lootCollector)
    {
      ...
      lootCollector.MakePrisonerHeroesEscape(1f);
    }
      ...
      int totalContribution)
    {
      ...
      lootCollector.GiveShareOfLootToParty(partyRec.RosterToReceiveLootMembers, partyRec.RosterToReceiveLootPrisoners, partyRec.RosterToReceiveLootItems, partyRec.Party, lootAmount);
    ...
    public void CalculateRenownAndValorValue(float[] strengthOfSide)
    {
      ...
      this.StrengthRatio = (float) (((double) strengthOfSide[oppositeSide] * Math.Sqrt((double) num1) + 10.0) / ((double) strengthOfSide[missionSide] * (double) num2 + 10.0));
      this.StrengthRatio = (double) this.StrengthRatio > 10.0 ? 10f : this.StrengthRatio;
      if ((double) strengthOfSide[missionSide] <= 0.0)
        return;
      this.RenownValue = (float) (Math.Pow((double) strengthOfSide[oppositeSide] * Math.Sqrt((double) num1), 0.75) * Math.Pow((double) this.StrengthRatio, 0.449999988079071) * (double) num3 * 0.75);
      this.InfluenceValue = (float) (Math.Pow((double) strengthOfSide[oppositeSide] * Math.Sqrt((double) num1), 0.75) * Math.Pow((double) this.StrengthRatio, 0.150000005960464) * 0.600000023841858);
      ...
      foreach (FlattenedTroopRosterElement troop in battleParty.Troops)
      {
        if (MapEventSide.TroopCanJoinBattle(troop))
        {
          ...
          else if (!troop.Troop.IsHero)
          {
            ...
            int num4 = (int) (100.0 / Math.Pow(1.20000004768372, (double) num2));
            if (num4 < 10)
              num4 = 10;
    ...
    internal void CollectAll(LootCollector lootCollector, out bool playerCaptured)
    {
      ...
      foreach (MapEventParty party in this.OtherSide.Parties)
      {
        ...
        if (party != null)
        {
          bool? perkValue = party.Party?.MobileParty?.Leader?.GetPerkValue(DefaultPerks.Roguery.RogueExtraordinaire);
          bool flag3 = true;
          if (perkValue.GetValueOrDefault() == flag3 & perkValue.HasValue && (double) num1 < (double) party.Party.MobileParty.Leader.GetSkillValue(DefaultSkills.Roguery))
          {
            num1 = (float) party.Party.MobileParty.Leader.GetSkillValue(DefaultSkills.Roguery);
            PerkHelper.AddEpicPerkBonusForCharacter(DefaultPerks.Roguery.RogueExtraordinaire, party.Party.MobileParty.Leader, DefaultSkills.Roguery, true, ref bonuses, 200);
          }
        }
      }
      foreach (MapEventParty battleParty in this._battleParties)
...
label_59:
      ...
      PartyBase strikerParty)
    {
      ...
      if (this._selectedSimulationTroop.IsHero)
      {
        ...
        {
          if ((double) MBRandom.RandomFloat > (double) Campaign.Current.Models.PartyHealingModel.GetSurvivalChance(this._selectedSimulationTroop.HeroObject.PartyBelongedTo?.Party ?? (PartyBase) null, this._selectedSimulationTroop, damageType, strikerParty) && this._selectedSimulationTroop.HeroObject.CanDie())
          {
    ...
    {
      this.OnTroopScoreHit(this._selectedSimulationTroopDescriptor, attackedTroop, damage, isFinishingStrike, false, (WeaponComponentData) null, true);
      PartyBase party = this._allocatedTroops[this._selectedSimulationTroopDescriptor].Party;
