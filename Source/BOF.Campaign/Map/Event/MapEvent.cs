...
namespace BOF.Campaign.Utility
{
  public sealed class MapEvent : MBObjectBase, IMapEntity
  {
      ...
      MapEvent.BattleTypes mapEventType = MapEvent.BattleTypes.None)
    {
      ...
      if (attackerParty.MobileParty != MobileParty.MainParty)
      {
        if (mapEventType == MapEvent.BattleTypes.Raid)
        {
          ...
          Debug.Print("[DEBUG] " + (object) attackerParty.Leader.Name + " is raiding " + (object) attackerParty.MobileParty.AiBehaviorObject.Name + "\n", color: Debug.DebugColor.DarkRed, debugFilter: 256UL);
        }
    ...
    public void AddInvolvedPartypublic(PartyBase involvedParty, BattleSideEnum side)
    {
      if (involvedParty.LeaderHero != null && involvedParty.LeaderHero.Clan == Clan.PlayerClan && involvedParty != PartyBase.MainParty && side == BattleSideEnum.Defender && this.AttackerSide.LeaderParty != null)
      {
        ...
        if (!flag)
        {
          ...
          float num1 = Campaign.MapDiagonal * Campaign.MapDiagonal;
      ...
      if (this.IsFieldBattle && involvedParty.IsMobile && involvedParty.MobileParty.BesiegedSettlement == null)
      {
        ...
        if (involvedParty != this.GetMapEventSide(side).LeaderParty)
        {
          ...
          involvedParty.MobileParty.EventPositionAdder = this.Position + vec2 * Math.Max(val1, 0.4f) + (float) ((num + 1) / 2 * (num % 2 == 0 ? 1 : -1)) * vec2.RightVec() * 0.4f - (involvedParty.Position2D + involvedParty.MobileParty.ArmyPositionAdder);
        }
        else
          involvedParty.MobileParty.EventPositionAdder = this.Position + vec2 * Math.Max(val1, 0.4f) - (involvedParty.Position2D + involvedParty.MobileParty.ArmyPositionAdder);
      }
    ...
    public void RemoveInvolvedPartypublic(PartyBase party)
    {
        ...
        foreach (MapEventSide side in this._sides)
        {
          ...
          for (int index = 0; index < array.Length; ++index)
          {
            ...
            if (party1.IsMobile && party1 != side.LeaderParty)
              party1.MobileParty.EventPositionAdder = this.Position + vec2 * Math.Max(val1, 0.4f) + (float) ((index + 1) / 2 * (index % 2 == 0 ? 1 : -1)) * vec2.RightVec() * 0.4f - (party1.Position2D + party1.MobileParty.ArmyPositionAdder);
          }
        }
      }
      ...
      float strikerAdvantage)
    {
      ...
      side1.ApplySimulatedHitRewardToSelectedTroop(allocatedTroop2, damage, isFinishingStrike);
    ...
    private bool GetAttackersRunAwayChance()
    {
      ...
      if (this.IsSiegeAssault)
        num1 *= 0.6666667f;
      return (double) num2 > (double) num1 * 1.10000002384186 && (double) MBRandom.RandomFloat * (this._mapEventUpdateCount < 16 ? Math.Sqrt((double) this._mapEventUpdateCount / 16.0) : 1.0) > (double) num1 / ((double) num2 * 1.10000002384186);
    }
        ...
        if (this._mapEventType == MapEvent.BattleTypes.Raid)
        {
          if (this.DefenderSide.TroopCount == 0)
          {
            ...
            if ((double) this._nextSettlementDamage > 0.05)
            {
              ...
              this.RaidDamage = Math.Min(this.RaidDamage, 1f);
              int num3 = 0;
              foreach (MapEventParty party in this.AttackerSide.Parties)
                num3 += party.Party.MemberRoster.TotalManCount;
              if (num3 > 0)
              {
                float num4 = (float) (1.0 - (double) Math.Max(0.0f, this.MapEventSettlement.SettlementHitPoints) / ((double) Math.Max(0.0f, this.MapEventSettlement.SettlementHitPoints) + (double) this._nextSettlementDamage));
                int num5 = 0;
                  ...
                  for (int index = 0; index < num6; ++index)
                  {
                    ...
                    if (!rosterElement.IsEqualTo(EquipmentElement.Invalid))
                    {
                      ...
                      if (this._lootedItemCount % 2 == 1)
                      {
                        ...
                        {
                          CharacterObject leader = partyBase.Leader;
                          if ((leader != null ? (leader.GetPerkValue(DefaultPerks.Steward.EfficientCampaigner) ? 1 : 0) : 0) != 0)
                            ++number;
                        }
