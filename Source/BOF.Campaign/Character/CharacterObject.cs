...
namespace BOF.Campaign.Character
{
  public sealed class CharacterObject : BasicCharacterObject, ICharacterData
  {
    ...
    private CharacterFeats _characterFeats;

    public override TextObject Name
    {
      get => this.IsHero ? this.HeroObject.Name : base.Name;
      set
      {
        if (this.IsHero)
          this.HeroObject.SetName(value);
        else
          base.Name = value;
      }
    }

    public string EncyclopediaLink => !this.IsHero ? Campaign.Current.EncyclopediaManager.GetIdentifier(typeof (CharacterObject)) + "-" + this.StringId : this._heroObject.EncyclopediaLink;
    ...
    public bool IsNotTransferableInPartyScreen => (this._characterRestrictionFlags & CharacterRestrictionFlags.NotTransferableInPartyScreen) == CharacterRestrictionFlags.NotTransferableInPartyScreen;

    public bool IsNotTransferableInHideouts => (this._characterRestrictionFlags & CharacterRestrictionFlags.CanNotGoInHideout) == CharacterRestrictionFlags.CanNotGoInHideout;
    ...
    public Hero HeroObject
    {
      ...
      public set => this._heroObject = value;
    }
    ...
    public Equipment FirstBattleEquipment => this.IsHero ? this.HeroObject.BattleEquipment : this.AllEquipments.FirstOrDefaultQ<Equipment>((Func<Equipment, bool>) (e => !e.IsCivilian));

    public Equipment FirstCivilianEquipment => this.IsHero ? this.HeroObject.CivilianEquipment : this.AllEquipments.FirstOrDefaultQ<Equipment>((Func<Equipment, bool>) (e => e.IsCivilian));

    public Equipment RandomBattleEquipment => this.IsHero ? this.HeroObject.BattleEquipment : this.AllEquipments.GetRandomElementWithPredicate<Equipment>((Func<Equipment, bool>) (e => !e.IsCivilian));

    public Equipment RandomCivilianEquipment => this.IsHero ? this.HeroObject.CivilianEquipment : this.AllEquipments.GetRandomElementWithPredicate<Equipment>((Func<Equipment, bool>) (e => e.IsCivilian));
    ...
    public override int MaxHitPoints() => MBMath.Round(Campaign.Current.Models.CharacterStatsModel.MaxHitpoints(this).ResultNumber);

    public ExplainedNumber MaxHitPointsExplanation => Campaign.Current.Models.CharacterStatsModel.MaxHitpoints(this, true);
    ...
    private void Init()
    {
      ...
      this._characterFeats = new CharacterFeats();
      this.Level = 1;
      ...
      bool ignoreSkillsAndTraits)
    {
      ...
      {
        characterObject.CharacterSkills = MBObjectManager.Instance.CreateObject<MBCharacterSkills>(characterObject.StringId);
      ...
      characterObject._characterFeats = new CharacterFeats(character._characterFeats);
    ...
    public override BodyProperties GetBodyPropertiesMin(bool returnBaseValue = false) => this.IsHero && !returnBaseValue ? this.HeroObject.BodyProperties : base.GetBodyPropertiesMin();

    public override BodyProperties GetBodyPropertiesMax() => this.IsHero ? this.HeroObject.BodyProperties : base.GetBodyPropertiesMax();
    ...
    public int GetUpgradeGoldCost(PartyBase party, int index) => Campaign.Current.Models.PartyTroopUpgradeModel.GetGoldCostForUpgrade(party, this, this.UpgradeTargets[index]);
    ...
    public void InitializeHeroCharacterOnAfterLoad()
    {
      if (this._originCharacter == null)
      {
        if (this._originCharacterStringId == null)
          return;
        this._originCharacter = MBObjectManager.Instance.GetObject<CharacterObject>(this._originCharacterStringId);
      }
    ...
    public override void Deserialize(MBObjectManager objectManager, XmlNode node)
    {
      ...
      foreach (XmlNode childNode1 in node.ChildNodes)
      {
        ...
        else if (childNode1.Name == "Feats" || childNode1.Name == "feats")
          this._characterFeats.Deserialize(objectManager, childNode1);
    ...
    private void SetCulturalFeats()
    {
      ...
      if (this.Culture == null)
        return;
      foreach (FeatObject attribute in Campaign.Current.Models.CharacterFeatsModel.GetAllFeatsForCulture(this.Culture))
        this._characterFeats.SetPropertyValue(attribute, 1);
    }

    private void ResetFeats() => this._characterFeats.ClearAllProperty();

    public override float GetPower()
    {
      int num = this.IsHero ? this.HeroObject.Level / 4 + 1 : this.Tier;
      return (float) ((double) ((2 + num) * (8 + num)) * 0.0199999995529652 * (this.IsHero ? 1.5 : (this.IsMounted ? 1.20000004768372 : 1.0)));
    }
      ...
      Equipment equipment = null)
    {
      ...
      for (EquipmentIndex index = EquipmentIndex.WeaponItemBeginSlot; index < EquipmentIndex.NumAllWeaponSlots; ++index)
      {
        ...
        {
          float num4 = equipmentElement1.Item.RelevantSkill == null ? 1f : (float) (0.300000011920929 + (double) this.GetSkillValue(equipmentElement1.Item.RelevantSkill) / 300.0 * 0.699999988079071);
          float val2 = num4 * equipmentElement1.Item.Effectiveness;
          if (equipmentElement1.Item.PrimaryWeapon.IsRangedWeapon)
          {
            for (EquipmentIndex equipmentIndex = EquipmentIndex.WeaponItemBeginSlot; equipmentIndex < EquipmentIndex.NumAllWeaponSlots; ++equipmentIndex)
            {
              ...
              {
                val2 += num4 * equipmentElement2.Item.Effectiveness;
                break;
              }
            }
          }
          if (equipmentElement1.Item.PrimaryWeapon.IsShield)
            defencePoints += val2 * 10f;
          else
      ...
      float num5 = equipment.Horse.Item == null || equipment.Horse.Item.RelevantSkill == null ? 1f : (float) (0.300000011920929 + (double) this.GetSkillValue(equipment.Horse.Item.RelevantSkill) / 300.0 * 0.699999988079071);
    ...
    public float GetHeadArmorSum(bool civilianEquipment = false) => !civilianEquipment ? this.FirstBattleEquipment.GetHeadArmorSum() : this.FirstCivilianEquipment.GetHeadArmorSum();

    public float GetBodyArmorSum(bool civilianEquipment = false) => !civilianEquipment ? this.FirstBattleEquipment.GetHumanBodyArmorSum() : this.FirstCivilianEquipment.GetHumanBodyArmorSum();

    public float GetLegArmorSum(bool civilianEquipment = false) => !civilianEquipment ? this.FirstBattleEquipment.GetLegArmorSum() : this.FirstCivilianEquipment.GetLegArmorSum();

    public float GetArmArmorSum(bool civilianEquipment = false) => !civilianEquipment ? this.FirstBattleEquipment.GetArmArmorSum() : this.FirstCivilianEquipment.GetArmArmorSum();

    public float GetHorseArmorSum(bool civilianEquipment = false) => !civilianEquipment ? this.FirstBattleEquipment.GetHorseArmorSum() : this.FirstCivilianEquipment.GetHorseArmorSum();
    ...
    public override BodyProperties GetBodyProperties(Equipment equipment, int seed = -1)
    {
      ...
      return FaceGen.GetRandomBodyProperties(this.IsFemale, this.GetBodyPropertiesMin(false), this.GetBodyPropertiesMax(), equipment != null ? (int) equipment.HairCoverType : 0, seed, this.HairTags, this.BeardTags, this.TattooTags);
    }

    public int TroopWage => this.IsHero ? 2 + this.Level * 2 : Campaign.Current.Models.PartyWageModel.GetCharacterWage(this.Tier);
    ...
    public int GetTraitLevel(TraitObject trait) => this.IsHero ? this.HeroObject.GetTraitLevel(trait) : this._characterTraits.GetPropertyValue(trait);

    public bool GetPerkValue(PerkObject perk) => this.IsHero && this.HeroObject.GetPerkValue(perk);

    public bool GetFeatValue(FeatObject feat) => (uint) this._characterFeats.GetPropertyValue(feat) > 0U;

    public void SetFeatValue(FeatObject feat, int value) => this._characterFeats.SetPropertyValue(feat, value);

    public override int GetSkillValue(SkillObject skill) => this.IsHero ? this.HeroObject.GetSkillValue(skill) : base.GetSkillValue(skill);
    ...
    {
      if (this.IsPlayerCharacter && this.Equipment != null)
      {
      ...
      return owner == PartyBase.MainParty && Campaign.Current.PlayerFormationPreferences.TryGetValue(this, out formationClass) ? formationClass : base.GetFormationClass(owner);
    }

    public static CharacterObject Find(string idString) => MBObjectManager.Instance.GetObject<CharacterObject>(idString);

    public static CharacterObject FindFirst(Predicate<CharacterObject> predicate) => CharacterObject.All.FirstOrDefault<CharacterObject>((Func<CharacterObject, bool>) (x => predicate(x)));
    ...
    public static MBReadOnlyList<CharacterObject> Templates => Campaign.Current.TemplateCharacters;

    public static MBReadOnlyList<CharacterObject> ChildTemplates => Campaign.Current.ChildTemplateCharacters;
  }
}