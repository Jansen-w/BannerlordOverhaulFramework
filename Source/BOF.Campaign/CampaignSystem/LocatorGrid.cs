      ...
      Func<T, bool> condition)
    {
      ...
      for (int xi = x; xi <= maxX; ++xi)
      {
        for (int yi = minY; yi <= maxY; ++yi)
        {
          ...
          for (curParty = this._nodes[this.MapCoordinates(xi, yi)]; (object) curParty != null; curParty = ((ILocatable<T>) (object) curParty).NextLocatable)
          {
            if ((curParty is Settlement settlement4 ? (double) settlement4.Position2D.DistanceSquared(position) : (double) ((object) curParty as MobileParty).Position2D.DistanceSquared(position)) < (double) r2 && condition(curParty))
              yield return curParty;
          }
          curParty = default (T);
        }
